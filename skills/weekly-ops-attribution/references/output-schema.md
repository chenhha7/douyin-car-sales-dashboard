...
drilldown.records                  evidence records matching the rule
verification                       question, evidence, scope, and interpretation boundary
```
...
| `dimension_analysis.rows` | `value`, current/prior metrics, `estimated_factor_impact` | Use for priority ranking; check sample size before interpretation. |
| `drilldown.records` | synthetic user/opportunity ID, focal dimensions, current stage, SLA duration, reason | Use as evidence for business review, never expose real personal data. |
| `verification` | `question`, `evidence`, `focus_dimension`, `note` | Present a hypothesis to verify; do not turn it into an assigned task or a strategy. |
...
4. **下钻**：`[n]` records meet `[drilldown rule]` and should be reviewed with the business team.
5. **待确认**：Ask `[verification question]`, state the evidence, and make clear that the result is not a confirmed root cause or a direct strategy recommendation.
